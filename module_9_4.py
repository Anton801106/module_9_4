...
# Даны 2 строки. Необходимо составить lambda-функцию для следующего выражения - list(map(?, first, second)).
...
write('Это строчка проверки', ['А', 'это', 'уже', 'число', 654, 'в', 'списке', 'для', 'проверки', 'решения'])

write = get_advanced_writer('xmpl.txt')
write('Это строчка', ['А', 'это', 'уже', 'число', 100, 'в', 'списке', 'для', 'проверки', 'решения'])

# Метод __call__:
# Создайте класс MysticBall, объекты которого обладают атрибутом words хранящий коллекцию строк.
# В этом классе также определите метод __call__ который будет случайным образом выбирать слово из words
# и возвращать его. Для случайного выбора с одинаковой вероятностью для каждого данного
...
second_ball = MysticBall('Всегда', 'Иногда', 'Часто', 'Редко', 'Никогда')
print(first_ball())
print(first_ball())
print(second_ball())
print(second_ball())